 ...
 */
'use strict';
var util = require("ui/url-util/url-util");
...
module.exports = {
    ...
    update: function (hash, options) {
        var newUrl,
            ...
            curHash = this._getCurrentUrl();
        if (newHash.replace(this.appPathReg, "") === curHash.replace(this.appPathReg, "")) {
            return;
        }
    ...
    navigate: function (url, options) {
        ...
        var curUrl = "/"+this._getCurrentUrl(),
            _options = options || {};

        _options.trigger = (_options.trigger !== false);

        // 如果配置了replace，则替换当前的历史记录
        if (_options.replace === true && _options.storageKey !== false) {
            try {
                window.localStorage.setItem(storageKey, curUrl);
            } catch (e) {
            }
    ...
    get: function (opts) {
        opts = opts || {};
        var path = opts.path || this._getCurrentUrl();
        var disableEncode = opts.disableEncode || false;
                ...
                seachbox: function (key) {
                    var ProcessDir = {
                        place: function (pkey) {
                            ...
                            pageState = 'from=place&' + pkey + '=' +
                                encodeURIComponent(pageStateArr[1]);
                        },
                        loc: function (lkey) {
    ...
    _getCurrentUrl: function () {
        return appPage.getFragment().replace(/^\//, "");
    },
    ...
    _get: function (newHash, options) {
        ...
        var curHash = this._getCurrentUrl(),
        ...
        if (pageStateReplace) {
            ...
            for (var i in newHash.pageState) {
                if (newHash.pageState[i] !== '') {
                    pageStateObject[i] = newHash.pageState[i];
                } else {
                    delete pageStateObject[i];
                }
            }
