...
$.extend(daytrip.prototype, {
    _setTpl: function() {
        var self = this;
        self.daytripTpl = '<h2 class="section-title">'+ daytripTitle +'</h2>'+
        '<ul>' +
        '{@each discountList as item, index}' +
            '<li class="discount-list J_daytrip-item {@if index < 5}show{@else}hideitem{@/if}" data-productId="${item.id}" data-src="${item.src}">' +
    ...
    loadData: function(sid){
        var self = this;
        var url = daytripUrl + sid + "&type=onedaytour&is_jsonp=1";
        $.ajax({
            url: url,
            dataType: 'jsonp',
            success: function(res){
                if (res.errno == 0) {
                    ...
                    if(res.data.list.length == 0) {
                        $('.daytrip').html('<h2 class="section-title">'+ daytripTitle +'</h2><div class="text">不好意思噢，当前城市没有特价资源，我们会火速争取！</div>');
                    }
                    else {
