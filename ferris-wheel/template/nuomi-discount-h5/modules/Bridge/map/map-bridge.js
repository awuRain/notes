...
var bridge = {
    ...
    pageConfig: {
        ...
        'skuDetail': {
            'map':'baidumap://map/component?comName=scenery&target=scenery_default_web_page_openapi'
        }
    },
        ...
        if(/sku/i.test(params.page)) {
            self.getNativeInfo(function(info) {
                if(info['sv'] && self._compareVersion(info['sv'], SKU_SUPPORT_VERSION)) {
                    ...
                    var notification = Notification.alert('百度地图版本过旧，您需要更新 ~~', function() {
                        this.hide();
                    });
                    notification.show();
                }
