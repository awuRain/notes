 ...
 */
var $ = require('zepto');
 var IScroll = require("ui/iscroll/iscroll");
(function (root) {
    /**
     * @namespace
     * @name jsmod
...
(function (root, $) {
    ...
    var Banner = jsmod.util.klass({
        initialize: function (option) {
            this.setStyle(option.style || jsmod.style["jsmod.ui.Banner"]);
        ...
        _bannerEvents: function () {
            ...
            self.$element.find("img").on("load", function () {
                var _w = this.width,
                    ...
                    _finalWidth, _finalHeight, maxRatio;

                maxRatio = Math.max(iwidth / _w, iheight / _h);
            ...
            self.on("active", function (e, idx) {
                self._loadIndex(idx);
                self.option.datas.length > 1 && self._autoRunFun(); // 只有大于 1 个才轮播
            });
            // 触发加载第一个
...
;(function (root, $) {    
    ...
    /** @lends jsmod.ui.Layer.prototype */
    {
        ...
        createIscroll: function () {
            ...
            // 只要需要使用 iscroll 则在显示时必会重置显示内容区域的高度
            if (option.isIScroll 
                    && this.$maskScreen.find(".mod-layer-content").length 
                    && !this.iscroll) {

                self.resetIScrollHeight();
                this.iscroll = new IScroll(self.$maskContent.find(".mod-dialog-iscroll-wrap").get(0), {
                    mouseWheel: true,
                    scrollbars: true
                });
            } else if (self.iscroll) {
        ...
        resetIScrollHeight: function () {
            ...
            // 重置
            iscrollWrap = self.$maskContent.find(".mod-dialog-iscroll-wrap");
            iscrollWrap.css("height", "auto");
            // debugger
            // 全部的高度
            allHeight = scrollHeight = self.$maskContent.height();

            // 处理头部内容
            if (self.$maskScreen.find(".mod-dialog-header-wrap")) {
                headerHeight = self.$maskScreen.find(".mod-dialog-header-wrap")
                        .height();
            ...
            // 处理底部内容
            if (self.$maskScreen.find(".mod-dialog-footer-wrap")) {
                footerHeight = self.$maskScreen.find(".mod-dialog-footer-wrap")
                        .height();
...
typeof(module) !== 'undefined' && module.exports ? module.exports = jsmod : this.jsmod = jsmod;