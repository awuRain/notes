 ...
 **/
lite.getImg = function(pic_url) {
    if (!pic_url) {
        return "";
    }
    var arr = ["http://hiphotos.baidu.com/", "lvpics", "/pic/item/", pic_url, ".jpg"];
    return arr.join("");
};
/**
 ...
 * @return {string} 空间图片url
 **/
lite.getAbImg = function(pic_url) {
    if (!pic_url) {
        return "";
    }
 ...
 * @return {string} 空间图片url
 **/
lite.getUserHead = function(avatar_url, avatar_source) {
 ...
 * @return {string} 空间图片url
 **/
lite.getUserAbHead = function(avatar_url, avatar_source) {
 ...
 */
lite.hilight = function(content, arrWords, color) {
    if (!color) {
        color = "#CC0000";
    }
    ...
    pat = pat.replace("/\1/", '|');
    pat = pat.replace("/\+/", '\+');
    content = $.string.encodeHTML(content);
    var pattern = "/^\.\.\./";
 ...
 **/
lite.cut = function(string, length, etc) {
    length = length || 40;
    etc = etc || "...";
    if (length === 0) {
        return '';
 ...
 * @return {Int} 用户等级
 */
lite.getUserLevel = function(score) {
 ...
 * @return {string} 用户头衔
 */
lite.getUserTitle = function(score) {
    var level = lite.getUserLevel(score);
 ...
 **/
lite.date = function(format, timestamp) {
 ...
 * @return {string} 格式化后的日期字符串
 **/
lite.feeddate = function(timestamp, feedformat, format) {
    /*
     feedformat = " H:i";
     format = " Y-m-d";
     timestamp = new Date();
    ...
    if ((timestamp > today) && (timestamp < tomorrow)) {
        return arr['today'] + lite.date(feedformat, timestamp);
    } else if (timestamp > yesterday) {
        return arr['yesterday'] + lite.date(feedformat, timestamp);
    } else {
        return lite.date(format, timestamp);
    }
};
 ...
 **/
lite.getTimeUnit = function(type) {
    var result = "";
    switch (type) {
        case "m":
            {
 ...
 * @return {string} 图片pid
 **/
lite.createImage = function($href, $url, $width, $height, $limitWidth, $limitHeight, $big, $withoutTarget) {
    var $is_full = $url.indexOf("http");
    if ($is_full != 0) {
        if ($big) {
            $url = lite.getImg($url);
        } else {
            $url = lite.getAbImg($url);
        }
    }
    var $targetAttr = '';
    if (!$withoutTarget) {
